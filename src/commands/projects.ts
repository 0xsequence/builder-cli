...
function checkAuth(json: boolean): void {
  if (!isLoggedIn()) {
    if (json) {
      console.log(
        JSON.stringify({error: "Not logged in", code: EXIT_CODES.NOT_LOGGED_IN})
      );
    } else {
...
async function listProjectsAction(options: {
  json?: boolean;
  env?: string;
  apiUrl?: string;
}) {
  ...
  try {
    ...
    if (!response.projects || response.projects.length === 0) {
      if (json) {
        ...
        console.log(
          chalk.gray('Create one with: sequence-builder projects create "My Project"')
        );
        console.log("");
      }
      return;
    ...
    console.log("");
    console.log(
      chalk.gray("Run `sequence-builder apikeys <project-id>` to view API keys")
    );
    console.log("");
  } catch (error) {
...
async function getProjectAction(
  ...
  try {
    ...
    if (response.project.chainIds?.length) {
      console.log(
        chalk.white("Chain IDs:   "),
        chalk.gray(response.project.chainIds.join(", "))
      );
    }
    console.log("");
  } catch (error) {
