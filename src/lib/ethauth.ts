...
export interface EthAuthClaims {
  app: string;
  exp: number;
  iat?: number;
  n?: number;
 ...
 */
function buildTypedData(claims: EthAuthClaims): {
  domain: typeof ETH_AUTH_DOMAIN;
  types: {Claims: Array<{name: string; type: string}>};
 ...
 */
function encodeClaimsToBase64(claims: EthAuthClaims): string {
...
export async function generateEthAuthProof(
  privateKey: string,
  customClaims?: Partial<Omit<EthAuthClaims, "v">>
): Promise<string> {
